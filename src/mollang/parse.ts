...
export default function parse(
    script: string
): [boolean, [number, number, number][] | string, number | number[]] {
    ...
    for (let i = 0; i < script.length; i++) {
        if (script[i] === "?" || script[i] === "!") {
            ...
            return [false, exitMessage(1, ".", [line_number, i]), 1];
            ...
            if (i + 1 < script.length && script[i + 1] === "?") {
                if (tokens[-1][0] === 8) {
                    ...
                    return [false, exitMessage(3, "루", [line_number, i]), 3];
                }
            } else {
                tokens.push([3, -1, -1]);
            }
            ...
            if (
                ...
                return [false, exitMessage(2, "?행", [line_number, i]), 2];
            }
            i += 2;
        } else if (script[i] === "털") {
            if (
                ...
                return [false, exitMessage(2, "?자", [line_number, i]), 2];
            }
            i += 2;
        } else if (script[i] === "0") {
            if (
                ...
                return [false, exitMessage(2, "ㅅ0", [line_number, i]), 2];
            }
        } else if (script[i] === "자") {
            if (i + 1 < script.length && script[i + 1] === "!") {
                tokens.push([7, -1, -1]);
            } else {
                return [false, exitMessage(2, "!", [line_number, i]), 2];
            }
        } else if (script[i] === "\n") {
            line_number++;
            line_backpoint.push(tokens.length);
        } else if (script[i] !== " ") {
            return [false, exitMessage(4, script[i], [line_number, i]), 4];
        }
    }
    return [true, tokens, line_backpoint];
}
